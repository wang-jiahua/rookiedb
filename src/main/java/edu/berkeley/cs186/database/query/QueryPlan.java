...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int column: columns) {
            SelectPredicate p = selectPredicates.get(column);
            boolean indexExists = transaction.indexExists(table, p.column);
            boolean canScan = p.operator != PredicateOperator.NOT_EQUALS;
            if (indexExists && canScan) {
                IndexScanOperator op = new IndexScanOperator(transaction, table, p.column, p.operator, p.value);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> set: prevMap.keySet()) {
            for (JoinPredicate p: joinPredicates) {
                String leftTable = p.leftTable;
                String rightTable = p.rightTable;
                String leftColumn = p.leftColumn;
                String rightColumn = p.rightColumn;
                QueryOperator op;
                ...
                if (result.containsKey(newSet)) {
                    if (op.estimateIOCost() < result.get(newSet).estimateIOCost()) {
                        result.put(newSet, op);
                    }
                } else {
                    result.put(newSet, op);
                }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> map = new HashMap<>(pass1Map);
